    ...
    */
    ...
    public static List<Earthquake> extractEarthquakes(String json) {
        ...
        try {
            ...
            // Create a JSONObject from the server response
            JSONObject jsonResponse = new JSONObject(json);

            // Create a JSONArray for all features (earthquakes)
            JSONArray earthquakeArray = jsonResponse.optJSONArray("features");

            // Loop through all the earthquakes
            for (int i = 0; i < earthquakeArray.length(); i++) {
                // Get the earthquake for this iteration
                JSONObject currentEarthquake = earthquakeArray.optJSONObject(i);
                // Get the earthquakes properties
                JSONObject properties = currentEarthquake.optJSONObject("properties");

                // Extract the magnitude, location, time and url from the earthquake
